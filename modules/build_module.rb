...
module BuildModule
  ...
  def run runner
    ...
    if runner.config['using_pods']
      ...
      system "xcodebuild -project \"#{runner.config['project']['name']}.xcodeproj\" -configuration \"#{runner.config['build']['configuration']}\" -sdk \"#{runner.config['build']['sdk']}\" -target \"#{runner.config['project']['target']}\" clean build CONFIGURATION_BUILD_DIR=\"#{runner.config['run']['build_dir']}\"" or fail "Build failed"
    end
